...
struct SimpleVertexShaderNoiseVertexColor
{
	__device__
	Vertex operator ()(const math::float3& v_p, const math::float3& v_n, math::float3& c) const
	{
		float noise = simplexNoise(v_p);
		//float noise = 0.5f * simplexNoise(p * 0.25f) + 0.25f * simplexNoise(p) + 0.25f * simplexNoise(p * 16.0f);
...
struct SimpleVertexShaderNoise
{
	__device__
...
struct SimpleFragmentShaderNoiseVertexColor : FragmentShader
{
	using FragmentShader::FragmentShader;
...
struct SimpleFragmentShaderNoise : FragmentShader
{
	using FragmentShader::FragmentShader;
	...
	{
		SimplexNoiseFractal<1> noise_fractal;

		float noise = noise_fractal(p);
